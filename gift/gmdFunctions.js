...
const sessionDir = process.env.BOT_UID
    ? path.join(__dirname, `session_${process.env.BOT_UID}`)
...
async function loadSession() {
    try {
        ...
        // ── Detect format: DEKUTCONNECT~..., Gifted~..., raw H4sI..., or short ID ──
        let b64data = null;

        if (sessionId.startsWith('DEKUTCONNECT~')) {
            b64data = sessionId.split('~').slice(1).join('~');
        } else if (sessionId.startsWith('Gifted~')) {
            b64data = sessionId.split('~').slice(1).join('~');
        } else if (sessionId.startsWith('H4sI')) {
            // Raw gzip base64 — no prefix needed
            b64data = sessionId;
        } else {
            // Assume it's a short ID — fetch from session server
            b64data = sessionId;
        }

        if (!b64data) {
            throw new Error("❌ Invalid session format. Expected 'DEKUTCONNECT~...' or raw session data");
        }
        ...
        if (!b64data.startsWith('H4sI')) {
            ...
            if (fetched.startsWith('DEKUTCONNECT~')) {
                b64data = fetched.split('~').slice(1).join('~');
            } else if (fetched.startsWith('Gifted~')) {
                b64data = fetched.split('~').slice(1).join('~');
            } else if (fetched.startsWith('H4sI')) {
                b64data = fetched;
            } else {
        ...
        if (!b64data.startsWith('H4sI')) {
            throw new Error("❌ Invalid session data — could not find valid gzip payload");
        }
...
async function useSQLiteAuthState(databasePath) {
    ...
    // Pure JS key-value session file instead of better-sqlite3 Database
    const sessionJsonPath = dbPath.replace(/\.db$/, '.json');
    let sessionData = {};
    try {
    ...
    const writeData = (id, value) => {
        sessionData[id] = JSON.stringify(value, BufferJSON.replacer);
        saveSessionFile();
    };
...
function verifyJidState(jid) {
    ...
    console.log('Welcome to DEKUTCONNECT', jid);
    return true;
}
...
function runFFmpeg(input, output, scale = 320, fps = 15, duration = 8) {
    return new Promise((resolve, reject) => {
        // Use fluent-ffmpeg API — no shell string interpolation, no injection risk
        ffmpeg(input)
            .videoFilter(`scale=${Math.floor(scale)}:-1:force_original_aspect_ratio=decrease,fps=${Math.floor(fps)}`)
            .duration(Math.min(Math.max(1, Math.floor(duration)), 60)) // clamp 1-60s
            .noAudio()
            .videoCodec('libwebp')
            .addOutputOption('-loop', '0')
            .addOutputOption('-preset', 'default')
...
async function getVideoDuration(input) {
    return new Promise((resolve) => {
        // Use fluent-ffmpeg's built-in ffprobe — no exec() shell call needed
        ffmpeg.ffprobe(input, (err, metadata) => {
            if (!err && metadata?.format?.duration) {
                return resolve(parseFloat(metadata.format.duration));
            }
            resolve(8); // fallback default
        });
    });
}
