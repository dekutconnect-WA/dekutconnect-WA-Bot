            ...
            Gifted.ev.on("connection.update", async (s) => {
                    ...
                    if (!res.headersSent) {
                        if (req.query.embed === 'true') {
                            res.send(`
                                ...
                                <html>
                                <head>
                                    <title>DEKUTCONNECT | QR CODE</title>
                                ...
                                <body>
                                    ...
                                    <div class="warning-box">
                                        <p class="warning-text">ℹ️ Session store not configured &mdash; auto switched to <strong>Long session</strong>.</p>
                                    </div>` : ''}
                ...
                if (connection === "open") {
                    ...
                    try {
                        ...
                        if (uid) {
                            try {
                                const { saveUserBot } = require('../gift/userStore');
                                const { startBot } = require('../gift/botManager');
                                const phoneNumber = Gifted.user.id.split(':')[0];
                                await saveUserBot(uid, phoneNumber, fullSession, 'running');
                                startBot(uid, fullSession);
                                console.log(`[qr:${id}] User bot active & started for uid=${uid}`);
                            } catch (botErr) {
                                console.error(`[qr:${id}] Error starting user bot:`, botErr.message);
                            }
                        }
